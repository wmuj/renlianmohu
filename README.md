...
- 📦 **极致批量处理** — 支持大量图片直接拖拽上传（含 50 张防崩溃保护）。处理完成后自动生成 ZIP 压缩包一键下载，文件名自动标注识别状态。
- ✨ **智能无损秒传** — 批处理中碰到“未包含人脸”的安全图片，会直接跳过 Canvas 压缩，100% 无图省画质原样打包进压缩包，极大提升效率。
- 🛡️ **防卡死异常隔离** — 完美拦截并跳过已损坏的图片，单张图的解析失败绝不会导致整批队列卡死罢工。
- 🎨 **椭圆模糊遮罩** — 以人脸为中心的椭圆形模糊，效果自然，边缘人脸遮罩计算精准无偏移。
